...
本リポジトリの内容は[ポートフォリオサイト](https://portfolio-s-sasaki-0529.vercel.app/)で一元管理することにしました。
以降の内容は念の為残しておきますが、古くなっている場合があります。

## 転職活動ステータス

**現職に満足しているので積極的な転職活動を行うつもりはないが、チャンスは見落としたくないというお気持ち**

※ 下記メールアドレスにご連絡を頂くこともありますが、興味が沸かない限りは返信は控えさせていただいてます
...
- ライブラリバージョンの管理と積極的な改善
  ...
  - フロントエンドビルド環境のアップグレード
    - node 8.x → 12.x → 14.x
    - Webpack 3 → 4 → 5
- JavaScript プロジェクトの TypeScript への全面移行をリード
- API ドキュメントの apiblueprint → Swagger への移行をリード
  - スキーマ駆動開発の基盤整備
    - 単体テストでのスキーマの品質担保
    - モックデータ生成
    - 型ファイル生成
